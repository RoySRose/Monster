...
public class PreBotUXManager {
	...
	public void update() {
	 ...
	 protected void drawUxInfo() {
	    	...
	    	uxDrawConfig= UxDrawConfig.newInstanceStringType("L","","* 1) UX Type Change		: d + num			ex) d1=?, d0=prebot1 display",UxColor.CHAR_WHITE);
	    	drawStrategyLeftList.add(uxDrawConfig);
	    	
	    	uxDrawConfig= UxDrawConfig.newInstanceStringType("L","","* 2) Change Strategy	: $ + Strategy Name	ex) $ INIT",UxColor.CHAR_WHITE);
	    	drawStrategyLeftList.add(uxDrawConfig);
	    	
	    	uxDrawConfig= UxDrawConfig.newInstanceStringType("L","","* 3) add Var 			: a + pos + class + var 	ex) a L(R/M) strategyBoard startStrategy",UxColor.CHAR_WHITE);
	    	drawStrategyLeftList.add(uxDrawConfig);
	    	
	    	uxDrawConfig= UxDrawConfig.newInstanceStringType("L","","* 4) minus Var 			: m + var  			ex) m startStrategy",UxColor.CHAR_WHITE);
	    	drawStrategyLeftList.add(uxDrawConfig);
	    ...
	    protected void drawTimer() {
	        ...
	        uxDrawConfig = UxDrawConfig.newInstanceStringType("M","",StrategyBoard.mainSquadMode.toString() + ": " + DrawingUtils.framesToTimeString(TimeUtils.getFrame()) + "(" + TimeUtils.getFrame() + ")",battleColor);
	    ...
	    protected void drawEnemyBuildTimer(){
	        ...
	        uxDrawConfig = UxDrawConfig.newInstanceStringType("L","engine Build Frame",DrawingUtils.framesToTimeString(StrategyBoard.engineeringBayBuildStartFrame),UxColor.CHAR_WHITE);
	    	drawStrategyLeftList.add(uxDrawConfig);
	    	
	    	uxDrawConfig = UxDrawConfig.newInstanceStringType("L","turret Build Frame",DrawingUtils.framesToTimeString(StrategyBoard.turretBuildStartFrame),UxColor.CHAR_WHITE);
	    	drawStrategyLeftList.add(uxDrawConfig);
	    	
	    	uxDrawConfig = UxDrawConfig.newInstanceStringType("L","turret Need Frame",DrawingUtils.framesToTimeString(StrategyBoard.turretNeedFrame),UxColor.CHAR_WHITE);
	    	drawStrategyLeftList.add(uxDrawConfig);
	    	
	    	uxDrawConfig = UxDrawConfig.newInstanceStringType("L","darkTemplarInMyBaseFrame",DrawingUtils.framesToTimeString(EnemyBuildTimer.Instance().darkTemplarInMyBaseFrame),UxColor.CHAR_WHITE);
	    	drawStrategyLeftList.add(uxDrawConfig);
	    	
	    	uxDrawConfig = UxDrawConfig.newInstanceStringType("L","reaverInMyBaseFrame",DrawingUtils.framesToTimeString(EnemyBuildTimer.Instance().reaverInMyBaseFrame),UxColor.CHAR_WHITE);
	    	drawStrategyLeftList.add(uxDrawConfig);
	    	
	    	uxDrawConfig = UxDrawConfig.newInstanceStringType("L","mutaliskInMyBaseFrame",DrawingUtils.framesToTimeString(EnemyBuildTimer.Instance().mutaliskInMyBaseFrame),UxColor.CHAR_WHITE);
	    	drawStrategyLeftList.add(uxDrawConfig);
	    	
	    	uxDrawConfig = UxDrawConfig.newInstanceStringType("L","lurkerInMyBaseFrame",DrawingUtils.framesToTimeString(EnemyBuildTimer.Instance().lurkerInMyBaseFrame),UxColor.CHAR_WHITE);
	    	drawStrategyLeftList.add(uxDrawConfig);
	    	...
	    	for (UnitType unitType : buildTimeExpectMap.keySet()) {
	            ...
	            if (buildTimeExpect != null && buildTimeExpect != CommonCode.UNKNOWN) {
	                ...
	                uxDrawConfig = UxDrawConfig.newInstanceStringType("L","",unitType + " : " + expect + " - min: " + minimum + " (" + buildTimeCertain.contains(unitType) + ")",UxColor.CHAR_WHITE);
	    	    	drawStrategyLeftList.add(uxDrawConfig);
	            }
	        }
	    ...
	    public void drawGameInformationOnScreen() {
	        uxDrawConfig = UxDrawConfig.newInstanceStringType("L","Current Strategy",StrategyBoard.currentStrategy.name(),UxColor.CHAR_WHITE);
	    	drawStrategyLeftList.add(uxDrawConfig);
	        ...
	        uxDrawConfig = UxDrawConfig.newInstanceStringType("L","MYKillScore",Broodwar.self().getKillScore(),UxColor.CHAR_RED);
	    	drawStrategyLeftList.add(uxDrawConfig);
	    	
	    	uxDrawConfig = UxDrawConfig.newInstanceStringType("L","MYRazingScore",Broodwar.self().getRazingScore(),UxColor.CHAR_RED);
	    	drawStrategyLeftList.add(uxDrawConfig);
	    	
	    	uxDrawConfig = UxDrawConfig.newInstanceStringType("L","EnemyKillScore",Broodwar.enemy().getKillScore(),UxColor.CHAR_PURPLE);
	    	drawStrategyLeftList.add(uxDrawConfig);
	     	
	     	uxDrawConfig = UxDrawConfig.newInstanceStringType("L","EnemyRazingScore",Broodwar.enemy().getRazingScore(),UxColor.CHAR_PURPLE);
	     	drawStrategyLeftList.add(uxDrawConfig);
	     	
	     	uxDrawConfig = UxDrawConfig.newInstanceStringType("L","Reserved Resource",ConstructionManager.Instance().getReservedMinerals()+ " / " + ConstructionManager.Instance().getReservedGas(),UxColor.CHAR_TEAL);
	     	drawStrategyLeftList.add(uxDrawConfig);
...
//	            }
	    ...
	    public void drawBuildOrderQueueOnScreen() {
	        ...
	        for (int i = 0; i < tempQueue.length; i++) {
		        ...
		        uxDrawConfig = UxDrawConfig.newInstanceStringType("M","",currentItem.blocking + " " + currentItem.metaType.getName(),UxColor.CHAR_WHITE);
		    	drawStrategyMidList.add(uxDrawConfig);
	            itemCount++;
	    ...
	    public void drawBuildStatusOnScreen()  {
	        ...
	        for (Unit unit : unitsUnderConstruction) {
	            ...
	            uxDrawConfig = UxDrawConfig.newInstanceStringType("R","","" +  t + " (" + unit.getRemainingBuildTime() + ")",UxColor.CHAR_WHITE);
	            drawStrategyRightList.add(uxDrawConfig);
	        }
	    ...
	    public void drawConstructionQueueOnScreenAndMap() {
	        ...
	        for (final ConstructionTask b : constructionQueue) {
	            if (b.getStatus() == ConstructionTask.ConstructionStatus.Unassigned.ordinal()) {
	            	uxDrawConfig = UxDrawConfig.newInstanceStringType("M","","" + b.getType() + " - No Worker",UxColor.CHAR_WHITE);
	    	    	drawStrategyMidList.add(uxDrawConfig);
	            } else if (b.getStatus() == ConstructionTask.ConstructionStatus.Assigned.ordinal()) {
	                if (b.getConstructionWorker() == null) {
	                	uxDrawConfig = UxDrawConfig.newInstanceStringType("M","",b.getType() + " - Assigned Worker Null",UxColor.CHAR_WHITE);
		    	    	drawStrategyMidList.add(uxDrawConfig);
	                } else {
	                	uxDrawConfig = UxDrawConfig.newInstanceStringType("M","",b.getType() + " - Assigned Worker " + b.getConstructionWorker().getID() + ", Position (" + b.getFinalPosition().getX() + "," + b.getFinalPosition().getY() + ")",UxColor.CHAR_WHITE);
	                	drawStrategyMidList.add(uxDrawConfig);
	                }
	            	...
	            	uxDrawConfig = UxDrawConfig.newInstanceStringType("M","","" + b.getType() + " - Under Construction",UxColor.CHAR_WHITE);
                	drawStrategyMidList.add(uxDrawConfig);
	            }
	        }
	    }
...
//	                AttackDecisionMaker.Instance()
	        ...
	        for (GameManager gameManager : gameManagers) {
	            ...
	            uxDrawConfig = UxDrawConfig.newInstanceStringType("R",gameManager.getClass().getSimpleName(),gameManager.getRecorded(),UxColor.CHAR_PURPLE);
		     	drawStrategyRightList.add(uxDrawConfig);
	        }
	        
	        uxDrawConfig = UxDrawConfig.newInstanceMethodType("R","* group size",LagObserver.class,  "groupsize",UxColor.CHAR_WHITE);
	     	drawStrategyRightList.add(uxDrawConfig);
	     	uxDrawConfig = UxDrawConfig.newInstanceMethodType("R","* manager rotation size",LagObserver.class,"managerRotationSize",UxColor.CHAR_WHITE);
	     	drawStrategyRightList.add(uxDrawConfig);
	    }
	    ...
	    protected void drawStrategy(){
	    	...
	    	//setting
	    	UxDrawConfig uxDrawConfig = UxDrawConfig.newInstanceStringType("L","", "[" + strategy.name() + " ...(phase " + phase + ")]",UxColor.CHAR_WHITE);
	    	drawStrategyLeftList.add(uxDrawConfig);
	    	
	    	uxDrawConfig = UxDrawConfig.newInstanceStringType("L","UPGRADE",upgradeString,UxColor.CHAR_WHITE);
	    	drawStrategyLeftList.add(uxDrawConfig);
	    	
	    	uxDrawConfig = UxDrawConfig.newInstanceStringType("L","EXPANSION",StrategyBoard.expansionOption,UxColor.CHAR_WHITE);
	    	drawStrategyLeftList.add(uxDrawConfig);
	    	
	    	uxDrawConfig = UxDrawConfig.newInstanceStringType("L","MISSION",strategy.missionTypeList,UxColor.CHAR_WHITE);
	    	drawStrategyLeftList.add(uxDrawConfig);
	    	
	    	uxDrawConfig = UxDrawConfig.newInstanceStringType("L","",ClueManager.Instance().getClueInfoList(),UxColor.CHAR_WHITE);
	    	drawStrategyLeftList.add(uxDrawConfig);
	    		...
	    		for (UxDrawConfig drwaConfig : drawStrategyListOrigin[uxOption]) {
	    			...
	    			if(drwaConfig.getValue() != null){
	    				...
	    				try{
	    					...
	    					Object obj = method.invoke(arg0, arg1);
		        		...
		        		uxDrawConfig = UxDrawConfig.newInstanceFieldType(fld.getName(),fld.get(drwaConfig.getValue()),drwaConfig.getColor());
