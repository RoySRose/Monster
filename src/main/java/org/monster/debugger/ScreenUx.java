...
public class ScreenUx{
	...
	public static void showDisplay() {
		...
		if (uxOption == 0) {
			...
			PreBotUXManager.Instance().drawUxInfo(); //debuging 메뉴 설명 (화면 , 속도 등)
        } else if (uxOption == 1) {
         ...
         // 미네랄PATH
	    	 ...
	    	 PreBotUXManager.Instance().drawConstructionQueueOnScreenAndMap(); //ConstructionQueue
	          ...
	          int mouseX = PreBotUXManager.Instance().Broodwar.getMousePosition().getX() + PreBotUXManager.Instance().Broodwar.getScreenPosition().getX();
	          int mouseY = PreBotUXManager.Instance().Broodwar.getMousePosition().getY() + PreBotUXManager.Instance().Broodwar.getScreenPosition().getY();
	          PreBotUXManager.Instance().Broodwar.drawTextMap(mouseX + 20, mouseY,
	                  "(" + (int) (mouseX / 32) + ", " + (int) (mouseY / 32) + ")");
	          PreBotUXManager.Instance().Broodwar.drawTextMap(mouseX + 20, mouseY + 10, "(" + (int) (mouseX) + ", " + (int) (mouseY) + ")");
		...
		/*try {
			PreBotUXManager.Instance().addDrawStrategyListOrigin();
