...
public class AdaptNewStrategy extends BaseBuild {
    ...
    public void adapt(EnemyStrategyOptions.ExpansionOption strategy, TilePosition factoryPos, TilePosition starport1, TilePosition starport2) {
        if (strategy == EnemyStrategyOptions.ExpansionOption.TWO_STARPORT) {
            int starportCount = UnitUtils.getUnitCount(CommonCode.UnitFindStatus.ALL_AND_CONSTRUCTION_QUEUE, UnitType.Terran_Starport);
            if (starportCount == 0) {
            ...
            int factoryCount = UnitUtils.getUnitCount(CommonCode.UnitFindStatus.ALL_AND_CONSTRUCTION_QUEUE, UnitType.Terran_Factory);
            if (factoryCount <= 1) {
